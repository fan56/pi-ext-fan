...
console.log(
  "=== 1. Within-COMPANIONS exact collisions (base/short identity) ===",
);
...
for (const n of names) {
  ...
  for (const [kind, v] of [
    ...
    if (seen.has(`${kind}:${v}`)) {
      withinFail++;
      reportFail(
        `${kind} "${v}" of "${n}" collides with "${seen.get(`${kind}:${v}`)}"`,
      );
    } else {
      seen.set(`${kind}:${v}`, n);
    }
...
if (withinFail === 0)
  reportOk(`no duplicate base/short across ${names.length} companions`);
...
for (const a of names) {
  for (const b of names) {
    ...
    if (base(A) === short(B) || short(A) === base(B)) {
      crossFail++;
      reportFail(
        `"${a}" (${base(A)}/${short(A)}) collides cross-wise with "${b}" (${base(B)}/${short(B)})`,
      );
    }
  }
}
if (crossFail === 0)
  reportOk("no cross base<->short collisions between companions");
...
for (const a of names) {
  for (const b of names) {
    ...
    if (
      short(A.pkg).length >= 3 &&
      short(A.pkg) !== short(B.pkg) &&
      short(A.pkg).length < short(B.pkg).length
    ) {
      if (
        base(B.pkg).includes(short(A.pkg)) ||
        short(B.pkg).includes(short(A.pkg))
      ) {
        reportInfo(
...
for (const reg of REGS) {
  const caught = names.filter((n) => {
    ...
    const pkg = c.pkg;
    return (
      reg.includes(pkg) || reg.includes(base(pkg)) || reg.includes(short(pkg))
    );
  });
  owners.set(reg, caught);
  if (caught.length > 1)
    reportFail(
      `registration "${reg}" caught by ${caught.length} companions: ${caught.join(", ")}`,
    );
  if (caught.length === 0)
    reportInfo(
      `registration "${reg}" matches no companion (third-party — must stay unmatched)`,
    );
}
...
console.log(
  "=== 5. Per-companion detection (simulates isCompanionInstalled) ===",
);
for (const n of names) {
  const c = COMPANIONS[n];
  if (!DETECT_ACTIVE(c)) {
  ...
  if (own.length === 0) {
    reportInfo(
      `${n} (${c.pkg}): no settings.json registration detected — NOT installed`,
    );
  } else {
...
console.log("=== 6. Spot checks ===");
{
  const thirdPartyHits = REGS.filter(
    (r) =>
      !names.some(
        (n) => DETECT_ACTIVE(COMPANIONS[n]) && owners.get(r)?.includes(n),
      ) &&
      names.some(
        (n) =>
          DETECT_ACTIVE(COMPANIONS[n]) &&
          (r.includes(short(COMPANIONS[n].pkg)) ||
            r.includes(base(COMPANIONS[n].pkg))),
      ),
  );
  if (thirdPartyHits.length === 0) {
    reportOk(
      "no third-party registration is caught by any companion short/base (0 false positives)",
    );
  } else {
...
console.log(
  fail === 0
    ? "COLLISION CHECK: \u2705 PASS (0 failures)"
    : `COLLISION CHECK: \u274c FAIL (${fail} failures)`,
);
